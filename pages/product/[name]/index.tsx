...
import { Description, Reviews } from "../../../components/product";
...
const Index: NextPage = ({ product }: any) => {
  ...
  return (
    <section
      className="container container-primary-px bg-lightGrey py-[3rem] mb-[3rem]"
      tabIndex={-1}
    >
      <div className="container container-primary-px bg-fifth pt-[3rem] pb-[10rem]">
        <article>
          ...
          <ul className="flex gap-[1.4rem]  ">
            <li
              className={
                "capitalize text-third text-[1.1rem] font-bold py-[.7rem] cursor-pointer border-t-[3px] " +
                `${
              ...
              onClick={handleChangeSection("description")}
            >
              description
            </li>
            <li
              ...
              onClick={handleChangeSection("reviews")}
            >
              reviews (1)
            </li>
          </ul>
...
export async function getStaticProps({ params }: contextI) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/products?${query}&populate=*`
  );
